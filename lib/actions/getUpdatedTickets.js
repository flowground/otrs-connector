...
async function processAction(msg, cfg, snp) {
    ...
    // load tickets that have been updated at the exact same date-time (second) as the last processed ticket (from prev execution)
    ...
    return otrs.getTickets(ids, filters).then(async tickets => {
        ...
        let since = moment(lastUpdatedTicketDate);
        ...
        for(const ticket of tickets) {
            ...
            ticket.Article.forEach(article => {
                article['_isNew'] = since.isBefore(article.Created);
            });
