 ...
 * @param {boolean} mod.processSkip - true to process the skip messages, false to ignore them (default)
...
function eioModule(mod) {
    ...
    mod.process = eioProcess(mod.process, mod.processSkip);
 ...
 * @param {boolean} processSkip - set to true to process the $SKIP messages as well
 * @returns {Function}
 */
function eioProcess(processFn, processSkip) {
    return function(msg, cfg, snapshot) {
        console.log('---PROCESS $SKIP:', msg.$SKIP);
        ...
        // emits a copy of a message, optionally with some fields modified
        this.emitCopy = (orig, overrides) => {
            let copy = Object.assign(messages.newEmptyMessage(), {
                body: orig.body,
                $SKIP: orig.$SKIP,
        ...
        // skip executing this action if so required, pass the input message to the next step
        if(!processSkip && msg.$SKIP) {
            this.emitCopy(msg);
            return Promise.resolve();
        }

        return Promise.resolve(processFn.call(this, msg, cfg, snapshot)).then(data => {
            console.log('---PROCESS returns: %j', data);
            if(data !== undefined) {
...
function eioRequest(method, path, options = {}) {
    ...
    let crtRequest = ++numRequests;
    console.log('---EIO REQUEST-%d: %s %s', crtRequest, req.method, req.uri.href, options.body);

    return req.then(res => {
        console.log('---EIO REQUEST-%d RESPONSE: %j', crtRequest, res);
        return res;
    }).catch(err => {
        console.log('---EIO REQUEST-%d ERROR: %j', crtRequest, err);
        return Promise.reject(err);
    });
}
