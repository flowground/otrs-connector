...
function eioRequest(method, path, options = {}) {
    ...
    numRequests ++;
    console.log('---MSP REQUEST-%d: %s %s', numRequests, req.method, req.uri.href);
    console.log('---MSP REQUEST-%d BODY: %s', numRequests, JSON.stringify(options.body));
...
async function createAttachment() {
    ...
    return {
        upload: (base64Content) => {
            ...
            return eioRequest('PUT', null, {body: attachBuffer, baseUrl: urls.put_url, json: false});
        },
...
function downloadAttachment(url) {
    return eioRequest('GET', null, {baseUrl: url, json: false, encoding: null});
}